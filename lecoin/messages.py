...
from lecoin.schema import Peer, Block, Transaction, Ping
...
class PeersMessage(Schema):
    ...
    @post_load
    def add_name(self, data: dict) -> dict:
        data["name"] = "peers"
        return data
...
def meta(ip: str, port: int, version: str = "lecoin-0.1") -> dict:
    return {
        "client": version,
        "address": {
            "ip": ip,
...
def create_peers_message(external_ip: str, external_port: int, peers) -> Any:
...
def create_block_message(external_ip: str, external_port: int, block) -> Any:
    return BaseSchema().dumps({
        "meta": meta(external_ip, external_port),
        "message": {
            "name": "block",
...
def create_transaction_message(external_ip: str, external_port: int, tx) -> Any:
    return BaseSchema().dumps({
        "meta": meta(external_ip, external_port),
        "message": {
            "name": "transaction",
